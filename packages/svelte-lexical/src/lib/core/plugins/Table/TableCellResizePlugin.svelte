<script lang="ts">
  ...
  let cellSizeBeforeDrag = $state(0);
      ...
      if (activeCell) {
        ...
        let styles = {
          horizontal: `
            ...
            cursor: col-resize;
            height: ${height}px;
            left: ${window.scrollX + left + width - threshhold / 2}px;
          ...
          vertical: `
            background-color: none;
            cursor: row-resize;
            height: ${threshhold}px;
            left: ${window.scrollX + left}px;
            top: ${window.scrollY + top + height - threshhold / 2}px;
            width: ${width}px;
          `,
        };
        ...
        if (draggingDirection && mouseCurrentPos && tableRect) {
          if (draggingDirection === 'horizontal') {
            styles.horizontal = `
              background-color: #219ebc8a;
              cursor: col-resize;
              height: ${tableRect.height}px;
              left: ${(window.scrollX + left + test - 2) / zoom}px;
          ...
          if (draggingDirection === 'vertical') {
            styles.vertical = `
              ...
              top: ${(window.scrollY + test - 2) / zoom}px;
    ...
    const handler = (event: PointerEvent) => {
      ...
      if (mouseStartPos) {
        ...
        if (direction === 'vertical') {
          ...
          test = cellSizeBeforeDrag + heightChange;
          updateRowHeight(cellSizeBeforeDrag + heightChange);
        }

        if (direction === 'horizontal') {
          const widthChange = (event.clientX - x) / zoom;
          test = cellSizeBeforeDrag + widthChange;
          updateColumnWidth(cellSizeBeforeDrag + widthChange);
        }
      }
    };
  ...
  function toggleResizeDirection(
    ...
    editor.update(() => {
      if (direction === 'horizontal') {
        const {currentCellWidth} = getRowWidths();
        cellSizeBeforeDrag = currentCellWidth;
      } else {
        const height = getRowHeight();
        cellSizeBeforeDrag = height;
      }
    });
